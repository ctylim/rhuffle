...
pub fn shuffle(conf: &Config) {
    let mut reader_dyn: Box<dyn BufRead> = match &conf.source {
        Some(source) => Box::new(io::reader(source.first().unwrap(), conf.io_buffer_size)),
        None => Box::new(BufReader::with_capacity(conf.io_buffer_size, stdin())),
    };
    let mut writer_dyn: Box<dyn Write> = match &conf.destination {
        Some(destination) => Box::new(io::writer(destination, conf.io_buffer_size)),
        None => Box::new(BufWriter::with_capacity(conf.io_buffer_size, stdout())),
    };
    head::forward_head(
        &mut reader_dyn,
        &mut writer_dyn,
    ...
    loop {
        ...
        match &conf.source {
            Some(source) => {
                ...
                if size == 0 {
                    reader_ind += 1;
                    if reader_ind < source.len() {
                        reader_dyn = Box::new(io::reader(&source[reader_ind], conf.io_buffer_size));
        ...
        let mut tmp_writer = io::writer(file.path().to_str().unwrap(), conf.io_buffer_size);
        for i in shuf {
    ...
    for tmp_file in tmp_files.iter() {
        tmp_file_readers.push(io::reader(tmp_file.file.path().to_str().unwrap(), conf.io_buffer_size));
    }
